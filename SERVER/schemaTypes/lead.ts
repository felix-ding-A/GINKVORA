...
export const leadType = defineType({
  ...
  fields: [
    ...
    defineField({ name: 'email', title: 'Email', type: 'string', validation: (Rule) => Rule.required().email(), readOnly: true }),
    defineField({ name: 'phone', title: 'Phone / WhatsApp', type: 'string', readOnly: true }),
    defineField({ name: 'interest', title: 'Interest', type: 'string', readOnly: true }),
    ...
    defineField({ name: 'demand', title: 'Annual demand', type: 'string', readOnly: true }),
    defineField({ name: 'application', title: 'Application', type: 'array', of: [{ type: 'string' }], readOnly: true }),
    defineField({ name: 'coaUrl', title: 'COA URL', type: 'url', readOnly: true }),
    ...
    defineField({ name: 'submittedAt', title: 'Submitted at', type: 'datetime', validation: (Rule) => Rule.required(), readOnly: true }),
    defineField({
      name: 'status',
      title: 'Lead status',
      type: 'string',
